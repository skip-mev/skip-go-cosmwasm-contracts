...
use cosmwasm_std::{
    to_binary, Api, BankMsg, Binary, Coin, CosmosMsg, DepsMut, Env, MessageInfo, OverflowError,
...
impl Asset {
    ...
    pub fn amount(&self) -> Uint128 {
        match self {
            Asset::Native(coin) => coin.amount.u128().into(),
    ...
    pub fn add(&mut self, amount: Uint128) -> Result<Uint128, SkipError> {
        match self {
            ...
            Asset::Cw20(coin) => {
                coin.amount = (coin.amount.u128() + amount.u128()).into();
    ...
    pub fn sub(&mut self, amount: Uint128) -> Result<Uint128, SkipError> {
        match self {
            ...
            Asset::Cw20(coin) => {
                if amount > coin.amount.u128().into() {
                    return Err(
                        OverflowError::new(OverflowOperation::Sub, amount, coin.amount).into(),
                    );
                }
                coin.amount = (coin.amount.u128() - amount.u128()).into();
                Ok(coin.amount.u128().into())
