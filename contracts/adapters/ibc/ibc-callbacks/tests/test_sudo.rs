...
use skip2::{ibc::IbcLifecycleComplete, sudo::OsmosisSudoMsg as SudoMsg};
use skip_go_ibc_adapter_ibc_callbacks::{error::ContractResult, state::ACK_ID_TO_RECOVER_ADDRESS};
...
#[test_case(
    Params {
        contract_balance: vec![Coin::new(Uint128::new(100), "uosmo")],
        channel_id: "channel_id".to_string(),
        sequence_id: 1,
        ...
        expected_messages: vec![
            SubMsg {
                ...
                msg: BankMsg::Send {
                    ...
                    amount: vec![Coin::new(Uint128::new(100), "uosmo")],
                }.into(),
                gas_limit: None,
                reply_on: Never,
            },
...
fn test_sudo(params: Params) -> ContractResult<()> {
    ...
    let res = skip_go_ibc_adapter_ibc_callbacks::contract::sudo(deps.as_mut(), env, params.sudo_msg);
