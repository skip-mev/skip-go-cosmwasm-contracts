...
use secret_skip::{ibc::IbcLifecycleComplete, sudo::OsmosisSudoMsg as SudoMsg};
use skip_go_secret_ibc_adapter_ibc_hooks::{
    error::ContractResult, state::ACK_ID_TO_RECOVER_ADDRESS,
};
...
fn test_sudo(params: Params) -> ContractResult<()> {
    ...
    let res =
        skip_go_secret_ibc_adapter_ibc_hooks::contract::sudo(deps.as_mut(), env, params.sudo_msg);
