use crate::{
    ...
    state::{ENTRY_POINT_CONTRACT_ADDRESS, RED_BANK_CONTRACT_ADDRESS},
};
use mars_red_bank_types::red_bank::{
    ExecuteMsg as MarsRedBankExecuteMsg, QueryMsg as MarsRedBankQueryMsg,
};
use cosmwasm_std::{
...
use skip::{
    asset::Asset,
    borrow::{
        execute_transfer_funds_back, MarsV1InstantiateMsg as InstantiateMsg, Cw20HookMsg,
...
pub fn instantiate(
    ...
    let checked_entry_point_contract_address = deps.api.addr_validate(
        &msg.entry_point_contract_address
    )?;
    ...
    let checked_red_bank_contract_address = deps.api.addr_validate(
        &msg.red_bank_contract_address
    )?;

    // Store the router contract address
    RED_BANK_CONTRACT_ADDRESS.save(deps.storage, &checked_red_bank_contract_address)?;
    ...
    Ok(Response::new()
        ...
        .add_attribute(
            "red_bank_contract_address",
            checked_red_bank_contract_address.to_string(),
        ))
}
...
pub fn receive_cw20(
    ...
    match from_binary(&cw20_msg.msg)? {
        Cw20HookMsg::Swap { operations } => execute_swap(
            deps, env, info, sent_asset, operations
        ),
    }
}
