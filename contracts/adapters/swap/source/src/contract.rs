...
use crate::swap_source;
use crate::swap_source::Hop;
use cosmwasm_std::{
...
fn execute_swap(
    ...
    let hops = operations.into_iter().map(|e| swap_source::Hop {
        pool: e.pool,
        denom: e.denom_out,
    });
    ...
    // Make the swap msg
    let swap_msg = swap_source::ExecuteMsg::SwapExactAmountInWithHops {
...
fn query_simulate_swap_exact_asset_out_with_metadata(
    ...
    let query_msg = swap_source::QueryMsg::SimulateSwapExactAmountOutWithHops {
        want_out: coin_out,
        hops,
    };
    ...
    let router_resp = deps
        .querier
        .query_wasm_smart::<swap_source::QuerySimulateSwapExactAmountOutWithHopsResponse>(
            router, &query_msg,
        )
        .map_err(|_| {
...
fn query_simulate_swap_exact_asset_in_with_metadata(
    ...
    let query_msg = swap_source::QueryMsg::SimulateSwapExactAmountInWithHops {
        input: coin,
        hops: operations
            .into_iter()
            .map(|op| Hop {
    ...
    let router_resp = deps
        .querier
        .query_wasm_smart::<swap_source::SwapExactAmountInWithHopsResponse>(router, &query_msg)?;
