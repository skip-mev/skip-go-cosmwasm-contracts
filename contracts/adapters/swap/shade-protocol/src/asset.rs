...
impl Asset {
    ...
    pub fn amount(&self) -> Uint128 {
        match self {
            ...
            Asset::Cw20(coin) => coin.amount.u128().into(),
        }
    }

    /*
    pub fn add(&mut self, amount: Uint128) -> Result<Uint128, ContractError> {
        match self {
            Asset::Native(coin) => {
            ...
            Asset::Cw20(coin) => {
                coin.amount = coin.amount.checked_add(amount.u128().into())?;
                Ok(coin.amount.u128().into())
            }
        }
    }

    pub fn sub(&mut self, amount: Uint128) -> Result<Uint128, ContractError> {
        match self {
            Asset::Native(coin) => {
            ...
            Asset::Cw20(coin) => {
                coin.amount = coin.amount.checked_sub(amount.u128().into())?;
                Ok(coin.amount.u128().into())
            }
        }
    }
...
mod tests {
    ...
    fn test_validate_native() {
        ...
        assert_eq!(res, Err(ContractError::InvalidNativeCoin));
    ...
    fn test_validate_cw20() {
        ...
        assert_eq!(
            res,
            Err(ContractError::Payment(PaymentError::NonPayable {}))
        );
        ...
        assert_eq!(res, Err(ContractError::InvalidCw20Coin));
    }
